...
export class ProfilePage implements OnInit {
  ...
  async getPhoto() {
    ...
    if (
      ...
      let butttondelete = {
        text: this.lang == 'en' ? 'Delete' : 'ବିଲୋପ କରନ୍ତୁ',
        handler: () => {
          this.deleteImage();
        },
      };
  ...
  deleteImage() {
    ...
    this.httpClient.post(url, params, { responseType: 'text' }).subscribe(
      (data) => {
        console.log('post data', params);
        this.user_data[0].user_image = this.default_img;
      },
      async (err) => {
        let alert = this.alertCtrl.create({
          header:
